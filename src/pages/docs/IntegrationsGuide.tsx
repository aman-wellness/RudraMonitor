// Step-by-step setup guide for every external platform Rudrans can talk to.
...
export default function IntegrationsGuide() {
  return (
    <DocsLayout
      ...
      subtitle="Connect Microsoft 365, Google Workspace, Slack, Teams, WhatsApp and other platforms to Rudrans — one platform per section."
      sections={sections}
      accent="cyan"
    >
      {/* 1. OVERVIEW */}
      <Section id="overview" title="1. Overview">
        <P>
          Rudrans connects to two families of external platforms — <strong>identity providers</strong>
        ...
        <Callout kind="info" title="One-time setup per platform">
          ...
          Rudrans dashboard. After that, real-time sync handles the rest — you won't need to touch
          these screens again unless tokens are rotated or you change provider.
        </Callout>
        <Sub title="Where to manage integrations in Rudrans">
          <Bullets items={[
      ...
      <Section id="m365" title="2. Microsoft 365 (Azure AD)">
        <P>
          Syncs every user + group in your tenant into Rudrans so you can manage employees,
        ...
        <Sub title="2.1 What you need">
          <Bullets items={[
            ...
            'No Azure App Registration setup — Rudrans uses its own multi-tenant app',
          ]} />
        </Sub>
        <Sub title="2.2 Setup steps">
          <Steps items={[
            <>In Rudrans, go to <code className="text-cyan-300">Employees → Integrations</code> → <strong>Connect Microsoft 365</strong>.</>,
        ...
        <Sub title="2.3 What you can do after connecting">
          <Bullets items={[
            'Edit a user\'s contact info (job title, office, phones, address, manager) in Rudrans — changes mirror to the M365 admin centre',
            'Assign managers in Rudrans — manager relationship is set in M365 too',
            'Add new employees from Rudrans → M365 user + license is created via Graph',
            'Send credentials, run offboardings, manage groups',
          ]} />
        </Sub>
        <Callout kind="warn" title="Permissions warning">
          The connection asks for <em>write</em> permissions because Rudrans creates / edits users.
      ...
      <Section id="google" title="3. Google Workspace">
        <P>
          Same shape as M365 — sync your Workspace users + groups into Rudrans, with full bi-directional
      ...
      <Section id="slack" title="4. Slack">
        ...
        <Sub title="4.1 Create the Slack app">
          <Steps items={[
            ...
            <>Name it <em>Rudrans</em> (or anything), pick your workspace, click <strong>Create App</strong>.</>,
        ...
        <Sub title="4.2 Add bot scopes + install">
          <Steps items={[
            ...
            <><strong>SKIP</strong> the "Token rotation" and "PKCE" sections at the top — Rudrans uses the simpler static-token pattern.</>,
...
users:read.email</Code>
        ...
        <Sub title="4.4 (Optional) Inbound replies — Event Subscriptions">
          <P>
            ...
            back to Rudrans), enable Event Subscriptions:
          </P>
          <Steps items={[
            <>Left sidebar → <strong>Event Subscriptions</strong> → toggle <strong>Enable Events</strong>.</>,
            <>In <strong>Request URL</strong>, paste:
              <Code>https://api-ems.rudrans.com/functions/v1/otp-inbound-slack?org=&lt;your-org-id&gt;</Code>
              Replace <code>&lt;your-org-id&gt;</code> with the value shown on your Rudrans OTP Settings page (a UUID).
            </>,
          ...
          <Callout kind="warn">
            ...
            "Approve in Rudrans" button instead of replying inside Slack.
          </Callout>
        </Sub>

        <Sub title="4.5 Plug into Rudrans">
          <Steps items={[
            <>Open <code className="text-cyan-300">ems.rudrans.com/employees/otp-settings</code> → <strong>Slack</strong> card.</>,
      ...
      <Section id="teams" title="5. Microsoft Teams">
        ...
        <Sub title="5.1 Easiest: Incoming Webhook (per-channel)">
          <Steps items={[
            ...
            <>Name it <em>Rudrans</em>, optionally upload an icon, click <strong>Create</strong>.</>,
            ...
            <>In Rudrans OTP Settings → Teams card → paste the webhook URL → Save.</>,
          ]} />
          <Callout kind="info">
            Webhook-only means one-way delivery (Rudrans → Teams). Users can't reply inside Teams.
        ...
        <Sub title="5.2 Full Graph integration (interactive)">
          <P>
            ...
            connected Microsoft 365 in Rudrans, Teams interactive mode is automatically available —
      ...
      <Section id="gchat" title="6. Google Chat">
        <P>
          Google Chat is bidirectional: Rudrans <strong>posts</strong> OTP cards into your space via a
          webhook, and admins <strong>reply</strong> inside the same space — the reply goes to the
          <em> Rudrans Chat App</em> which fulfills the pending OTP request in real-time. No magic-link
          round-trip.
        </P>
        <Callout kind="info" title="Two pieces to wire up">
          ...
          OTP cards from Rudrans → Chat. <br />
          (b) <strong>Inbound: add the Rudrans Chat App</strong> — once-per-space click to install. Lets
          admins reply with the OTP code inside Chat → Rudrans.
        </Callout>
        ...
        <Sub title="6.1 Outbound — Incoming webhook (5 min, one-time per space)">
          <Steps items={[
            ...
            <><strong>Add webhook</strong> → name it <em>Rudrans OTP</em> → <strong>Save</strong>.</>,
            ...
            <>In Rudrans: <code className="text-cyan-300">Employees → OTP Channels → Google Chat → Outbound webhook URL</code> → paste → leave Space ID for now.</>,
          ]} />
        </Sub>
        ...
        <Sub title="6.2 Find your Space ID">
          <Steps items={[
            ...
            <>Paste it into Rudrans → Google Chat card → <strong>Space ID</strong> → <strong>Save</strong>.</>,
          ]} />
          <Callout kind="warn">
        ...
        <Sub title="6.3 Inbound — Add the Rudrans Chat App to your space">
          <P>
            We publish a Chat App called <strong>Rudrans OTP</strong> in Google Workspace Marketplace.
            One-click install:
          </P>
          <Steps items={[
            <>In the space, click <strong>Apps &amp; integrations</strong> → <strong>+ Add apps</strong>.</>,
            <>Search for <strong>Rudrans OTP</strong> → click <strong>Add</strong>.</>,
            <>Approve the prompt. The bot will post a "Hi, I'm online" message into the space.</>,
            <>That's it. The bot listens for messages in this space (and only this space — Google scopes Chat App events to where it's added) and routes them back to your Rudrans org via the Space ID you saved above.</>,
          ]} />
          <Callout kind="info" title="What admins do during an OTP">
            When Rudrans posts an OTP card (e.g. "Acme platform needs a code"), the assigned admin
            just replies in the space with the 6-digit code. Within 1–2 seconds the bot answers
            "✅ OTP received and applied" and Rudrans completes the workflow.
          </Callout>
        </Sub>

        <Sub title="6.4 Verify end-to-end">
          <Steps items={[
            <>In Rudrans Vault → trigger a credential fetch that needs an OTP.</>,
        ...
        <Sub title="6.5 Common errors">
          <KV k="Bot says 'no org has linked this space yet'" v="You added the bot but didn't paste the Space ID into Rudrans. Copy the ID the bot mentioned and paste it under Google Chat → Space ID, then Save." />
          <KV k="Bot doesn't reply at all" v="Make sure you added the Rudrans OTP app to the space (step 6.3), not just the webhook. Webhook = outbound only." />
          <KV k="Outbound card never arrives" v="Webhook URL expired or webhook deleted in the space. Recreate via Manage webhooks and paste the new URL into Rudrans." />
        </Sub>
      </Section>
      ...
      <Section id="whatsapp" title="7. WhatsApp">
        ...
        <Sub title="7.1 Meta Cloud API">
          <Steps items={[
            ...
            <>In Rudrans OTP Settings → WhatsApp card: provider = <strong>Meta Cloud</strong>, paste the Phone Number ID + access token + approved template name → Save.</>,
          ]} />
        </Sub>
        <Sub title="7.2 Twilio">
          <Steps items={[
            ...
            <>In Rudrans: provider = <strong>Twilio</strong>, paste the three values → Save.</>,
          ]} />
        </Sub>
        <Callout kind="warn">
      ...
      <Section id="razorpay" title="8. Razorpay (billing)">
        <P>
          ...
          renewals. Most customers don't need to configure this — Rudrans uses its own Razorpay
          account by default.
        </P>
        <Sub title="When you might need your own Razorpay">
          <Bullets items={[
            'You\'re a Rudrans channel partner reselling licenses and want collections to land in your own bank account',
      ...
      <Section id="troubleshooting" title="9. Troubleshooting">
        ...
        <Sub title="OTP test message doesn't arrive">
          <KV k="Slack" v="Channel ID mismatch (must be C0XXX… not the channel name), or bot not invited to a private channel (run /invite @Rudrans in the channel)." />
        ...
        <Sub title="Microsoft 365 sync shows 404 for a group">
          <P>
            ...
            Rudrans now skips missing groups silently — no action needed. Run "Sync now" if you want
            the group removed from Rudrans immediately.
          </P>
        </Sub>
        <Sub title="Still stuck?">
          <P>
            Email <code className="text-cyan-300">support@rudrans.com</code> with the platform name +

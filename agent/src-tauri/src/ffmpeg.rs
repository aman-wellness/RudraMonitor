...
pub fn encoder_args(encoder: &str) -> Vec<&'static str> {
    ...
    // The bitstream profile MUST match what WebRTC negotiates on the agent
    // side. webrtc-rs's register_default_codecs() offers/answers H.264
    // constrained-baseline (profile-level-id=42e01f) with
    // level-asymmetry-allowed=1 — and nothing else. If the encoder emits a
    // different profile, Chrome receives RTP but its baseline-negotiated
    // decoder produces ZERO frames: a live connection with a black
    // rectangle (input still works — it rides a separate DataChannel).
    //
    // We shipped MAIN @ 4.0 (v0.6.8–v0.7.13). That only worked while a
    // dashboard-side SDP munger rewrote the offer to main; when Live moved
    // to LiveKit that munger was deleted, so main silently regressed to a
    // black screen on every Remote session. Constrained-baseline is the
    // WebRTC-mandatory profile (RFC 7742) and universally decodable.
    //
    // The old "baseline caps at 720p" note was a LEVEL confusion, not a
    // profile one: baseline profile supports level 4.0 (1080p30) fine, and
    // level-asymmetry-allowed=1 lets our level-4.0 bitstream ride the
    // level-3.1 negotiated line. `-bf 0` keeps the zero-latency contract.
    match encoder {
